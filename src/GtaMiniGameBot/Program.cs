...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        if (args.Length > 0 && args[0].Equals("--verify-capture", StringComparison.OrdinalIgnoreCase))
        {
            Native.AttachConsole(Native.ATTACH_PARENT_PROCESS);
            TrySetUtf8Console();
            var report = new StringWriter();
            ...
            int rc;
            try { rc = VerifyCapture.Run(args); }
            catch (Exception ex) { Console.WriteLine("LOI: " + ex); rc = 3; }

            TryWriteUtf8(Path.Combine(AppContext.BaseDirectory, "verify-capture.txt"), report.ToString());
            return rc;
        }

        if (args.Length > 0 && args[0].Equals("--verify-survival", StringComparison.OrdinalIgnoreCase))
        {
            Native.AttachConsole(Native.ATTACH_PARENT_PROCESS);
            TrySetUtf8Console();
            var report = new StringWriter();
            ...
            int rc;
            try { rc = VerifySurvival.Run(args); }
            catch (Exception ex) { Console.WriteLine("LOI: " + ex); rc = 3; }

            TryWriteUtf8(Path.Combine(AppContext.BaseDirectory, "verify-survival.txt"), report.ToString());
            return rc;
        }
